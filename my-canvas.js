...
(function () {
    ...
    MyCanvas.prototype = {
        ...
        _createDom: function () {
            ...
            document.addEventListener('mousemove', this._domMove)
        ...
        // 鼠标移上去
        _swiperBoxDomMouseOver: function() {
            _myCanvasThis.swiperBoxDom.classList.add("active")
        },
        _clearSwiperBoxDom: function() {
            _myCanvasThis.swiperBoxDom.style.cssText = ""
            _myCanvasThis.swiperBoxDom.children[0].style.cssText = ""
            _myCanvasThis.swiperBoxDom.children[0].children[0].children[0].style.cssText = ""
        },
        // 刷新组件状态
        ...
        _initComponentStatus: function (status) {
            ...
            // setTimeout(()=> {
            //     _myCanvasThis._clearSwiperBoxDom()
            // }, 1000)
        },
        _imgSwiperStart: function (e) {
            _myCanvasThis.overflowLock = true
        ...
        _imgSwiperEnd: function (e) {
            ...
            if (!_myCanvasThis._checkedRange(e)) {
                if(_myCanvasThis.overflowLock) {
                    _myCanvasThis.overflowLock = false
                }else {
                    return
                }
            }
            ...
            if (Math.abs(_myCanvasThis.sCanvas.style.left.split("px")[0] - (_myCanvasThis.orgX - 2)) <= 8) {
                ...
                _myCanvasThis.swiperBoxDom.classList.add("error")
        ...
        _domMove: function (e) {
            if(!_myCanvasThis.overflowLock) return
            const moveX = e.clientX - _myCanvasThis.sX
            _myCanvasThis._swiperBoxDomMouseOver()
            if(e.target.nodeName === "CANVAS") {
                if (moveX < 0 || moveX + _myCanvasThis.L >= _myCanvasThis.w) {
                    return false
                }
                ...
                if (moveX < 0 || moveX + _myCanvasThis.sw >= _myCanvasThis.w) {
                    return false
                }
                let sL = ((_myCanvasThis.w - _myCanvasThis.L) / (_myCanvasThis.w - _myCanvasThis.sw)) * moveX
                _myCanvasThis.sCanvas.style.left = sL + 'px'
                _myCanvasThis.swiperDom.style.left = moveX + 'px'
                _myCanvasThis.swiperBoxDom.style.width = moveX + _myCanvasThis.sw + 'px'
